  ...
  <BasicModal v-bind="$attrs" @register="registerModal" title="修改密码" @ok="handleSubmit">
    <a-form class="antd-modal-form" ref="formRef" :model="formState" :rules="validatorRules">
      ...
      <a-form-item name="password">
        ...
        <span class="gray-9e font-size-13">8位以上数字、大小写字母和特殊符号组成!</span>
      </a-form-item>
    </a-form>
  </BasicModal>
</template>
<script lang="ts" name="user-pass-word-modal" setup>
  ...
  const validatorRules: Record<string, Rule[]> = {
    password: [{ required: true, validator:checkPassword},{ pattern:/^(?=.*?[a-z])(?=.*?[A-Z])(?=.*?\d)(?=.*?[~!@#$%^&*()_+`\-={}:";'<>?,./]).{8,}$/,message:'密码由8位数字、大小写字母和特殊符号组成!'}],
  ...
  async function handleSubmit() {
    try {
      ...
      await updateUserPassword(values).then((res) =>{
        ...
        $message.createMessage.warn(res.message || '密码修改失败');
      });
    } finally {
      setModalProps({ confirmLoading: false });
    }
