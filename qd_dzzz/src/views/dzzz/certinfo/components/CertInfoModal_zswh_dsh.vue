<template>
  <BasicModal v-bind="$attrs" @register="registerModal"  destroyOnClose :title="'证照信息1'" :showOkBtn="false" :width="1200"
             >
...
<!--    <div v-if="iswgz">
    ...
    <a-button  style="float:right;margin-right: 10px;width: 100px;height: 50px" @click="bcak">退回</a-button>
    <a-button @click="shenhetongguo" style="background-color: #0a8fe9;color: azure;float: right;margin-right: 10px;width: 100px;height: 50px">审核通过</a-button>
...
import CertQXGZ from '../components/CertSHBTG.vue'
...
import {
  ...
  certchildYlqxwlxsbaColumns, bgjlcolumns
} from '../CertInfo.data';
import {
  saveOrUpdate,
  certchildYpscZcList,
  ...
  getshiyanlist,
  certchildYlqxwlxsbaList, certchildDylylqxscbapzList, bgxxlist, shenhequest, back
} from '../CertInfo.api';
...
const [registerFormwh, {setProps:setPropswh,  validate:validatewh,setFieldsValue:setwhxx}] = useForm({
  labelWidth: 120,
  schemas: formSchemawhjy,
  showActionButtonGroup: false,
  baseColProps: {span: 24}
});
...
//企业名称不可修改
  ...
  if (unref(isUpdate)) {
    ...
    let promise1 =await bgxxlist({zsid:data.record.zsid});
    await  setwhxx({
      ...promise1.records[0]
    });
    requestSubTableData(certchildYpscZcList, {id: data?.record?.zsid,zt: data?.record?.zt}, certchildYpscZcTable)
    requestSubTableData(certchildYpscWtList, {id: data?.record?.zsid,zt: data?.record?.zt}, certchildYpscWtTable)
    requestSubTableData(certchildYpscStList, {id: data?.record?.zsid,zt: data?.record?.zt}, certchildYpscStTable)
    requestSubTableData(certchildYlqxwtbaZbList, {id:data?.record?.zsid,zt: data?.record?.zt}, certchildYlqxwtbaZbTable)
    requestSubTableData(certchildYlqxscbaZbList, {id:data?.record?.zsid,zt: data?.record?.zt}, certchildYlqxscbaZbTable)
    requestSubTableData(certchildYpckxkzList, {id:data?.record?.zsid,zt: data?.record?.zt}, certchildYpckxkzTable)
    requestSubTableData(certchirdYlqxlcsybaList, {id:data?.record?.zsid,zt: data?.record?.zt}, certchirdYlqxlcsybaTable)
    requestSubTableData(certchildYlqxcpckxsList, {id:data?.record?.zsid,zt: data?.record?.zt}, certchildYlqxcpckxsTable)
    requestSubTableData(certchildMzyphjsypddscList, {id:data?.record?.zsid,zt: data?.record?.zt}, certchildMzyphjsypddscTable)
    requestSubTableData(certchildYpckxszmList, {id:data?.record?.zsid,zt: data?.record?.zt}, certchildYpckxszmTable)
    requestSubTableData(certchildYpscGmpList, {id:data?.record?.zsid,zt: data?.record?.zt}, certchildYpscGmpTable)
    requestSubTableData(certchildCkomylyList, {id:data?.record?.zsid,zt: data?.record?.zt}, certchildCkomylyTable)
    requestSubTableData(certchildYlqxwlxsbaList, {id:data?.record?.id,zt: data?.record?.zt}, certchildYlqxwlxsbaTable)
    requestSubTableData(certchildDylylqxscbapzList, {id:data?.record?.id,zt: data?.record?.zt}, certchildDylylqxscbapzTable)
  }
 ...
 * 审核通过
 */
async function  shenhetongguo()
{
  try {
    setModalProps({confirmLoading: true});
    await shenhequest(formdatas.value);
    closeModal();
    //刷新列表
    emit('success');
  }
