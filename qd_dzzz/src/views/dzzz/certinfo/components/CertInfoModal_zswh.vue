...
<!--    <div>
    ...
    <div>
      ...
      <div style="margin-top: 20px;margin-bottom: 20px;"><span style="font-size: 20px">操作记录修改</span></div>
      <div>
        <JVxeTable
          keep-source
          resizable
...
import {
  ...
  getshiyanlist,
  certchildYlqxwlxsbaList, certchildDylylqxscbapzList, bgxxlist
} from '../CertInfo.api';
...
import {updateRecordAndDetial} from "@/views/dzzz/operation/CertInfoOperationRecord.api";
// Emits声明
const nu=ref(false);
...
//表单配置
const [registerForm, {setProps, resetFields, setFieldsValue,resetSchema,appendSchemaByField,removeSchemaByFiled,validate}] = useForm({
  labelWidth: 120,
  schemas: formSchema,
...
//企业名称不可修改
  ...
  if (unref(isUpdate)) {
    ...
    console.log(promise1.records);
    bgjl.dataSource=promise1.records;
...
async function requestAddOrEdit(values) {
  try {
    ...
    values.fj=promise.fj;
    values.certtypeid=certtypID;
    values.id=certid;
    values.zt=uuid.zt;
    values.yxzt='10';
    values.certoperationrecord=bgjldata.value!.getTableData();
/*
    await updateRecordAndDetial(bgjl.dataSource);
*/
    if(isbu)
    {
      values.isbl=isbu;
