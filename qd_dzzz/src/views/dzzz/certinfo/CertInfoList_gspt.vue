  ...
  <div>
    ...
    <BasicTable :row-selection="rowSelection" @register="registerTable">
      ...
      <template #action="{ record }">
        <TableAction :actions="getDropDownActionYUlan(record)" />
      </template>
      <!--字段回显插槽-->
...
<script lang="ts" name="dzzz.certinfo-certInfo" setup>
  ...
  const { prefixCls, tableContext, onExportXls, onImportXls } = useListPage({
    tableProps: {
      ...
      beforeFetch: (params) => {
        ...
        if (props.data.zsdl) {
          params.zsdl = props.data.zsdl;
          params.zt = props.data.zt;
        } else {
        }
  ...
  function getDropDownActionYUlan(record) {
    return [
      ...
      {
        label: '副页',
        onClick: openYulan.bind(null, record, '附页'),
      },
      {
        label: '变更记录',
        onClick: openYulan.bind(null, record, '变更'),
      },
    ];
  }
</script>
