...
const Gspt_Page = PageEnum.Gspt_Page;
...
const whitePathList: PageEnum[] = [LOGIN_PATH, OAUTH2_LOGIN_PAGE_PATH,SYS_FILES_PATH, TOKEN_LOGIN,Gspt_Page ];
...
export function createPermissionGuard(router: Router) {
  ...
  router.beforeEach(async (to, from, next) => {
    ...
    //单点登录
    if (to.query.code) {
      //if (token) await userStore.logout();
      await userStore.oauth2Login({ code: to.query.code + '', goHome: false });
      next(userStore.getUserInfo.homePath);
      return;
    }
