 ...
 * @returns {Promise<any>}
 */
export async function validateFormModelAndTables(validate, formData, cases, props, autoJumpTab?) : Promise<{formValue: Recordable, tablesValue: any[]}> {
  ...
  let subData: unknown =[];
  if(cases!=null) {
     subData = await validateTables(cases, autoJumpTab);
  }
  // 合并最终数据
...
export function validateTables(cases, autoJumpTab = true) {
  ...
  return new Promise((resolve, reject) => {
    ...
    (function next() {
      let vm = cases[index];
      if (isRef(vm)) vm = vm.value;
      if(vm!=null) {
        vm.validateTable().then((errMap) => {
          // 校验通过
          if (!errMap) {
            tablesData[index] = {tableData: vm.getTableData()};
