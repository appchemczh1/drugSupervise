...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    props: {
      ...
      style: propTypes.any,
      valueLimit: propTypes.oneOfType([propTypes.string, propTypes.array])
    },
    emits: ['options-change', 'change','update:value', 'update:showText'],
    setup(props, { emit, refs }) {
      ...
      async function initDictData() {
        let { dictCode, stringToNumber, valueLimit } = props;
        //根据字典Code, 初始化字典数组
        const dictData = await initDictOptions(dictCode);
        dictOptions.value = dictData.reduce((prev, next) => {
          if (next && (!valueLimit || (Array.isArray(valueLimit) ? valueLimit.includes(next['value']) : (',' + valueLimit + ',').includes(',' + next['value'] + ',')))) {
      ...
      function handleChange(e, option) {
        ...
        emit('update:showText', getShowText(changeValue))
      ...
      function handleChangeRadio(e) {
        ...
        emit('update:showText', getShowText(e?.target?.value ?? e))
        //update-end---author:wangshuai ---date:20230504  for：【issues/506】JDictSelectTag 组件 type="radio" 没有返回值------------
      }

      function getShowText(value) {
        const { mode } = unref<Recordable>(getBindValue);
        let array = dictOptions.value.filter(i => Array.isArray(value) ? value.includes(i.value) : value == i.value).map(i => i.label);
        return mode === 'multiple' ? array : (0 < array.length ? array[0] : '');
      }
