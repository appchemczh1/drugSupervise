 ...
 * @Version: V1.0
 */
@ApiModel(value="certchild_ypsc_zc对象", description="药品生产子表自产")
@Data
@TableName("certchild_ypsc_zc")
public class    CertchildYpscZc implements Serializable {
...
//    private java.util.Date createTime;
	...
	public String getFullProductionScope() {
		String[] array = {"/", "\\", "-", "~", "无", "N/A"};
		String str = getProductionscope(), str2 = getProductionscopeF();
		if (StringUtils.equalsAny(str, array)) str = "";
		if (StringUtils.equalsAny(str2, array)) str2 = "";
		if (StringUtils.isNotBlank(str) && StringUtils.isNotBlank(str2)) return str + "（" + str2 + "）";
		if (StringUtils.isNotBlank(str)) return str;
		if (StringUtils.isNotBlank(str2)) return str2;
		return "";
	}
