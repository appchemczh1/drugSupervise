...
<script lang="ts" setup>
    ...
    import {formSchema,certchildYlqxwlxsbaColumns,certchildDylylqxscbapzColumns} from '../CertInfo.data';
    ...
    const [registerModal, {setModalProps, closeModal}] = useModalInner(async (data) => {
        ...
        if (unref(isUpdate)) {
            ...
            await setFieldsValue({
             ...
             requestSubTableData(certchildYlqxwlxsbaList, {id:data?.record?.id}, certchildYlqxwlxsbaTable)
             requestSubTableData(certchildDylylqxscbapzList, {id:data?.record?.id}, certchildDylylqxscbapzTable)
        }
        // 隐藏底部时禁用整个表单
    ...
    async function reset(){
      await resetFields();
      activeKey.value = 'certchildYlqxwlxsba';
