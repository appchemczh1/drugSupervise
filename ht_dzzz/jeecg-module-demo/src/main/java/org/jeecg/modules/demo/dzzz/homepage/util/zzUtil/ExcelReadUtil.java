   ...
   private static final int CELL_TYPE_ERROR = 5;
    ...
    private static List<Map> parseExcel(Sheet sheet, List<String> fieldNames,  Integer rowNum, Integer rowEnd, Integer colNum,Map extendMap) throws IllegalAccessException, InstantiationException {
        ...
        for (int r = rowNum; r <= rowEnd; r ++) {
            ...
            if (org.jeecg.modules.demo.dzzz.certStatistics.util.zzUtil.ValidateUtil.isEmpty(resultData)) {
    ...
    private static Map convertRowToData(Row row, Integer colNum, List<String> fieldNames,Map extendMap) {
        ...
        if(!org.jeecg.modules.demo.dzzz.certStatistics.util.zzUtil.ValidateUtil.isEmpty(extendMap)){
    ...
    private static String convertCellValueToString(Cell cell) {
        ...
        switch (cell.getCellType()) {
            ...
            case STRING:
                //字符串
                returnValue = org.jeecg.modules.demo.dzzz.certStatistics.util.zzUtil.ValidateUtil.isEmpty(cell.getStringCellValue()) ? "" : cell.getStringCellValue().trim();
                break;
            case BOOLEAN:
                //布尔
