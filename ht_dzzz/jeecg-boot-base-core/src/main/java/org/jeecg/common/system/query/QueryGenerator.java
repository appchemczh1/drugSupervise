...
public class QueryGenerator {
	...
	public static <T> QueryWrapper<T> initQueryWrapper(T searchObj,Map<String, String[]> parameterMap) {
		return initQueryWrapper(searchObj, parameterMap, new String[0]);
	}

	public static <T> QueryWrapper<T> initQueryWrapper(T searchObj,Map<String, String[]> parameterMap, String... ignoreProperties) {
		long start = System.currentTimeMillis();
		QueryWrapper<T> queryWrapper = new QueryWrapper<T>();
		installMplus(queryWrapper, searchObj, parameterMap, ignoreProperties);
	 ...
	 */
	private static void installMplus(QueryWrapper<?> queryWrapper,Object searchObj,Map<String, String[]> parameterMap, String... ignoreProperties) {
		...
		*/
		...
		for (int i = 0; i < origDescriptors.length; i++) {
			...
			try {
				if (judgedIsUselessField(name)|| !PropertyUtils.isReadable(searchObj, name) || ArrayUtils.contains(ignoreProperties, name)) {
					continue;
				}
				if (Class.forName("org.jeecg.modules.demo.dzzz.certinfo.entity.ACertInfo").isAssignableFrom(searchObj.getClass())
					&& "certTypeId".equals(name)) {
					continue;
				}
