...
public class LoginController {
	...
	@ApiOperation("oauth2登录接口")
	@PostMapping("/oauth2Login")
	public Result<JSONObject> oauth2Login(@RequestBody JSONObject jsonObject, HttpServletRequest request) {
		Result<JSONObject> result = new Result<JSONObject>();
		String code = jsonObject.getString("code");
		//校验用户有效性
		String url = "http://192.168.0.233/UIAS-api";
		Map<String, String> params = new HashMap<>(),reparam= new HashMap<>();
		params.put("appid", "1822181628894519297");
		params.put("secret", "IqrNvlVLGzJSIyFae7b7nSjxooxYxasz");
		params.put("code", code);
		params.put("grant_type", "authorization_code");
		JSONObject json = null;
		JSONObject userjson = null;
		try {
			json = JSON.parseObject(OkHttpUtils.getJson(url + "/oauth2/access_token", params));
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			return Result.error("获取token失败！请联系后台管理员");
		}
		if (!json.getBooleanValue("success")) {
			return Result.error(StringUtils.defaultIfBlank(json.getString("message"), "code错误"));
		}
		json = json.getJSONObject("result");
		reparam.put("access_token",json.getString("access_token"));

		SysUser sysUser = SsoUtils.findUser(json, new SsoUtils.SsoContext());
		boolean isCreateUser = null == sysUser, isUpdateUser = !isCreateUser && (null == sysUser.getRemoteUpdateTime() || json.getDate("updateTime").after(sysUser.getRemoteUpdateTime()));
		if (isCreateUser || isUpdateUser) {
			try {
				userjson = JSON.parseObject(OkHttpUtils.getJson(url+ "/oauth2/userinfo", reparam));
				userjson = userjson.getJSONObject("result");
			} catch (Exception e) {
				log.error(e.getMessage(), e);
				return Result.error("获取用户详情失败！请联系后台管理员");
			}
		}
		if(isUpdateUser){
			Set<String> roles = new LinkedHashSet<>(), departs = new LinkedHashSet<>();
			SsoUtils.writeUserInfo(sysUser, roles, departs, userjson);
			sysUserService.editUser(sysUser, StringUtils.join(roles, ","), StringUtils.join(departs, ","), "0");
		} else if(isCreateUser){
			Set<String> roles = new LinkedHashSet<>(), departs = new LinkedHashSet<>();
			SsoUtils.writeUserInfo(sysUser = new SysUser(), roles, departs, userjson);
			sysUserService.saveUser(sysUser, StringUtils.join(roles, ","), StringUtils.join(departs, ","), "0");
			sysUser = SsoUtils.findUser(json, new SsoUtils.SsoContext());
			if (null == sysUser) return Result.error("用户不存在");
		}
